...
class PuntoVentaHeladeria(DisenoHeladeria):
    ...
    def mostrar_empleados(self):
        ...
        tk.Label(
            frame,
            text="Seleccione nombre de empleado:",
            font=FUENTE_NORMAL,
            bg=COLOR_FONDO
        ).pack()
        ...
        if not empleados:
            lista.insert(tk.END, "No se encontraron empleados")

        def seleccionar():
            seleccion = lista.curselection()
            if not empleados:
                messagebox.showwarning("Aviso", "No hay empleados cargados desde la base de datos")
                return
            if not seleccion:
            ...
            self.empleado = empleados[seleccion[0]]
            self.mostrar_validar_empleado()

        self.boton(frame, "Siguiente", seleccionar).pack(pady=5)
        self.boton(frame, "Regresar", self.mostrar_inicio).pack(pady=5)

    def mostrar_validar_empleado(self):
        self.limpiar()
        frame = self.marco_centrado()
        self.titulo(frame)
    ...
    def _mostrar_punto_venta_anterior(self):
        self.limpiar()
        ...
        tk.Label(
            frame,
            text=f"Cliente: {self.cliente_nombre}",
            font=FUENTE_NORMAL,
            fg=COLOR_TEXTO,
            bg=COLOR_FONDO
        ).place(x=25, y=55)
        ...
        panel.place(x=25, y=90, width=430, height=560)
        ...
        self.txt_comentarios.pack(padx=10, pady=(0, 10))

        controles = tk.Frame(panel, bg=COLOR_FONDO)
        controles.pack()

        self.boton(controles, "Quitar", self.quitar_producto, 10).grid(row=0, column=0, padx=5)
        self.boton(controles, "+", self.aumentar_producto, 4).grid(row=0, column=1, padx=5)
        self.boton(controles, "-", self.disminuir_producto, 4).grid(row=0, column=2, padx=5)
        ...
        self.lbl_total.pack(pady=18)

        pagos = tk.Frame(panel, bg=COLOR_FONDO)
        pagos.pack()

        self.boton(pagos, "Efectivo", self.pagar_cash, 10).grid(row=0, column=0, padx=8)
        self.boton(pagos, "Tarjeta", self.pagar_card, 10).grid(row=0, column=1, padx=8)

        self.boton(frame, "Nueva orden", self.mostrar_nueva_orden, 12).place(x=25, y=670)
        self.boton(frame, "Salir", self.mostrar_inicio, 12).place(x=170, y=670)

        menu_frame = tk.Frame(frame, bg=COLOR_FONDO)
        menu_frame.place(x=500, y=90, relwidth=0.62, height=780)
        ...
        ancho_imagen_menu = 170
        alto_imagen_menu = 125
